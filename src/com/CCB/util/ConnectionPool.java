...
public class ConnectionPool implements AppLog{
...
//	private static ConnectionPool connPool = null;
	...
	static {
		...
		try {
			db_config.load(ins);
			jdbcDriver = db_config.getProperty("DB_Driver");
			dbUrl = db_config.getProperty("DB_URL");
			dbUsername = db_config.getProperty("user");
			dbPassword = db_config.getProperty("password");
			// 创建数据库连接库对象
		} catch (IOException e) {
			e.printStackTrace();
	...
	private static class SingletonClassInstance {
		private static final ConnectionPool instance = new ConnectionPool(jdbcDriver, dbUrl, dbUsername, dbPassword); 
	} 
	 ...
	 * 
	 */
	private ConnectionPool(String jdbcDriver, String dbUrl, String dbUsername, String dbPassword) {
	}
	...
	private Connection newConnection() throws SQLException {
		...
		if (connections.size() == 0) {
			...
			// 连接限制，或数据库的最大连接限制不知道
			// driverMaxConnections 为返回的一个整数，表示此数 据库允许客户连接的数目
